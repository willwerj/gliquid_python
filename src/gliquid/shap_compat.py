...
def patch_xgb_base_score() -> None:
    ...
    try:
        ...
        logger.debug("shap not installed – skipping patch_xgb_base_score")
        return

    tree_mod.float = _safe_float  # type: ignore[attr-defined]
    _APPLIED.add(tag)
    logger.debug("Applied shap_compat patch: %s", tag)
...
def patch_waterfall_no_twin_axes() -> None:
    ...
    try:
        ...
        logger.debug("shap not installed – skipping patch_waterfall_no_twin_axes")
        return
...
def unpatch_waterfall_no_twin_axes() -> None:
    ...
    try:
        ...
        logger.debug("Could not unpatch waterfall_no_twin_axes", exc_info=True)
    _APPLIED.discard(tag)
